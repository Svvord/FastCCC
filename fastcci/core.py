...
def statistical_analysis_method(
    ...
    # Staget V : Calculate Cluster Markers based on Scores for clusters and Null Distribution
    #start = timeit.default_timer()
    #clustermarker_pvals = pd.DataFrame(
    #    list(map(get_pvalue_from_pmf, mean_counts.stack(), mean_pmfs.stack())), # map can only deal with Series
    ...
    ## TODO: decide whether adding a id->name dictionary into database file or not..
    #stop = timeit.default_timer()
    # print(stop-start) # 8.264961563050747 seconds

    # start = timeit.default_timer()
    # clustermarker_pvals = pd.DataFrame(
    #     mean_counts.stack().combine(mean_pmfs.stack(), lambda x, y: get_pvalue_from_pmf(x, y)),
    #     index=mean_counts.stack().index
    # ).unstack()
    # stop = timeit.default_timer()
    # print(stop-start) # 15.327024035155773 seconds
